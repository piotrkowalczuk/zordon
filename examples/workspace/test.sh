...
# workspace — each service gets its OWN sparse per-service checkout, so
...
# A named workspace checks out HEAD. If the service sources aren't
...
for s in serviceA serviceB serviceC; do
	git -C "$ROOT" cat-file -e "HEAD:examples/workspace/src/$s/main.go" 2>/dev/null \
		|| skip "examples/workspace/src/$s not committed (a @HEAD workspace can't see it); commit the sources to run this"
done
...
assert_present "$EXDIR/workspaces/feature/src/serviceC/.git"
zordon workspace service rm --workspace=feature --services=serviceC
assert_absent "$EXDIR/workspaces/feature/src/serviceC"
assert_present "$EXDIR/workspaces/feature/src/serviceA/.git"
zordon workspace service add --workspace=feature --services=serviceC
assert_present "$EXDIR/workspaces/feature/src/serviceC/.git"
b="$(git -C "$EXDIR/workspaces/feature/src/serviceC" rev-parse --abbrev-ref HEAD)"
[ "$b" = "zordon/feature/serviceC" ] \
	&& pass "serviceC re-added on its own branch ($b)" \
	|| fail "serviceC expected branch zordon/feature/serviceC, got '$b'"

cd "$EXDIR/workspaces/feature"
...
# Workspace structure: each service gets its OWN sparse per-service
# checkout under workspaces/feature/src/<svc> that carries only
# its own subtree and excludes the sibling services' sources.
base="$EXDIR/workspaces/feature/src"
all="serviceA serviceB serviceC"
for svc in $all; do
	co="$base/$svc"
	assert_present "$co/.git"
	assert_present "$co/examples/workspace/src/$svc/main.go"
	for other in $all; do
		[ "$other" = "$svc" ] && continue
		assert_absent "$co/examples/workspace/src/$other"
	done
done
